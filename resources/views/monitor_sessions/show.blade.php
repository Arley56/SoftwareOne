    ...
    <div class="container-fluid">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
            <div>
                ...
                <p class="text-secondary mb-0">Información general de la monitoría seleccionada.</p>
            </div>

            @if(auth()->user()?->roles?->name === 'Monitor')
                <a href="{{ route('monitor.dashboard') }}" class="btn btn-outline-light rounded-pill px-4">Volver</a>
            @else
                <a href="{{ route('monitor-sessions.index') }}" class="btn btn-outline-light rounded-pill px-4">Volver</a>
            @endif
        </div>
        ...
        <div class="card border-0 shadow-lg rounded-4 overflow-hidden bg-dark mb-4">
            <div class="card-header bg-dark border-secondary px-4 py-3">
                <h5 class="mb-0 text-light">Tabla: Detalle de sesión</h5>
            </div>
            <div class="table-responsive">
                <table class="table table-dark table-bordered mb-0 align-middle">
                    <tbody>
                        <tr>
                            <th class="text-nowrap" style="width: 220px;">ID</th>
                            <td>{{ $session->id }}</td>
                        </tr>
                        <tr>
                            <th class="text-nowrap">Monitor</th>
                            <td>
                                {{ $session->schedule->monitor->user->name ?? 'Sin monitor' }}
                                <br>
                                <small class="text-secondary">Sem {{ $session->schedule->monitor->semestre ?? '' }}</small>
                            </td>
                        </tr>
                        <tr>
        ...
        @if(auth()->user()?->roles?->name === 'Monitor')
            <div class="card border-0 shadow-lg rounded-4 overflow-hidden bg-dark mb-4">
                <div class="card-header bg-dark border-secondary px-4 py-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div>
                        <h5 class="mb-1 text-light">Cargar material de apoyo</h5>
                        <small class="text-secondary">Sube archivos locales para complementar la formación de los estudiantes.</small>
                    </div>
                </div>
                <div class="card-body px-4 py-4">
                    <form action="{{ route('monitor-sessions.materials.store', $session->id) }}" method="POST" enctype="multipart/form-data" class="d-flex flex-column flex-md-row align-items-md-center gap-3">
                        @csrf
                        <input type="file" name="material" id="material" class="d-none" onchange="this.form.submit()" required>
                        <label for="material" class="btn btn-success rounded-pill px-4 fw-semibold mb-0">
                            Cargar material de apoyo
                        </label>
                        <small class="text-secondary">Selecciona el archivo desde tu computador y se cargará automáticamente.</small>
                    </form>
                    @error('material')
        ...
        <div class="card border-0 shadow-lg rounded-4 overflow-hidden bg-dark mb-4">
            <div class="card-header bg-dark border-secondary px-4 py-3 d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-1 text-light">Panel: Material de apoyo</h5>
                    <small class="text-secondary">Archivos cargados para esta monitoría.</small>
                </div>
                <span class="badge bg-info text-dark">{{ $session->sessionMaterials->count() }} archivos</span>
            </div>
            <div class="table-responsive">
                <table class="table table-dark table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($session->sessionMaterials as $material)
                            <tr>
                                <td class="ps-4 fw-semibold">{{ $material->original_name }}</td>
                                <td>{{ $material->uploader->name ?? 'Sin usuario' }}</td>
                                ...
                                <td class="text-end pe-4">
                                    @if(auth()->user()?->roles?->name === 'Monitor')
                                        <div class="d-inline-flex gap-2 flex-wrap justify-content-end">
                                            ...
                                            <form action="{{ route('monitor-sessions.materials.destroy', [$session->id, $material->id]) }}" method="POST" onsubmit="return confirm('¿Eliminar este archivo?')" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center text-secondary py-5">Todavía no se han cargado materiales de apoyo para esta sesión.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="modal-dialog modal-dialog-centered modal-xl">
                <div class="modal-content bg-dark text-light border border-secondary">
                    ...
                    <div class="modal-body p-0">
                        <div id="materialPreviewBody" class="bg-black" style="min-height: 70vh;"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-lg rounded-4 overflow-hidden bg-dark">
            <div class="card-header bg-dark border-secondary px-4 py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 text-light">Tabla: Estudiantes inscritos</h5>
                <span class="badge bg-warning text-dark">{{ $session->sessionEnrollments->count() }} inscritos</span>
            </div>
            <div class="table-responsive">
                <table class="table table-dark table-hover align-middle mb-0">
                    <thead class="border-bottom border-secondary">
                    ...
                    <tbody>
                        @forelse($session->sessionEnrollments as $enrollment)
                            <tr>
                                <td class="ps-4 fw-semibold">{{ $enrollment->user->name ?? 'Sin nombre' }}</td>
                                <td>{{ $enrollment->user->email ?? 'Sin correo' }}</td>
                                <td>
                                    <span class="badge bg-success text-white">{{ ucfirst($enrollment->status ?? 'activa') }}</span>
                                </td>
                                <td>{{ $enrollment->enrolled_at ? \Illuminate\Support\Carbon::parse($enrollment->enrolled_at)->format('d/m/Y H:i') : $enrollment->created_at?->format('d/m/Y H:i') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="text-center text-secondary py-5">No hay estudiantes inscritos en esta sesión.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
